...
impl_pert_multichain_getter!(
    tinned_two_elec_energy_derivative : TwoElecEnergy => |op| Ok(Arc::new(op.derivative().clone()))
);

// Get `dependencies` (cloned).
impl_pert_multichain_getter!(
    tinned_two_elec_energy_dependencies : TwoElecEnergy => |op| Ok(Arc::new(op.dependencies().clone()))
);
