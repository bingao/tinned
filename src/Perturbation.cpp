...
namespace Tinned
{
    ...
    int Perturbation::compare(const SymEngine::Basic& o) const
    {
        ...
        if (get_name()==s.get_name()) {
            if (SymEngine::is_a_Number(*frequency_) &&
                SymEngine::is_a_Number(*s.frequency_)) {
                int result;
                // Some subclasses of SymEngine::Number cannot be compared
                // directly, so we take their difference and compare
                auto diff = SymEngine::subnum(
                    SymEngine::rcp_dynamic_cast<const SymEngine::Number>(frequency_),
                    SymEngine::rcp_dynamic_cast<const SymEngine::Number>(s.frequency_)
                );
                if (diff->is_complex()) {
                ...
                return frequency_->__cmp__(*s.frequency_);
            }
        }
        else {
