...
TEST_CASE("Test two-level system", "[TwoLevelFunction], [TwoLevelOperator]")
{
    ...
    // D^{abc} = [Vc^{c}, D^{ab}]\circ[(omega_a+omega_b+omega_c-(E_m-E_n))^{-1}]
    //         + [Vb^{b}, D^{ac}]\circ[(omega_a+omega_b+omega_c-(E_m-E_n))^{-1}]
    //         + [Va^{a}, D^{bc}]\circ[(omega_a+omega_b+omega_c-(E_m-E_n))^{-1}]
    ...
    val_freq_denom = SymEngine::immutable_dense_matrix(
        2, 2,
        {
            SymEngine::div(SymEngine::one, SymEngine::add({omega_a, omega_b, omega_c})),
            SymEngine::div(
                SymEngine::one,
    ...
    // D^{aab} = 2*[Va^{a}, D^{ab}]\circ[(2*omega_a+omega_b-(E_m-E_n))^{-1}]
    //         + [Vb^{b}, D^{aa}]\circ[(2*omega_a+omega_b-(E_m-E_n))^{-1}]
    auto val_Daa = oper_eval.apply(D->diff(a)->diff(a));
    auto val_freq_aba = SymEngine::immutable_dense_matrix(
        2, 2,
        {
            SymEngine::div(
                SymEngine::two, SymEngine::add({omega_a, omega_a, omega_b})
            ),
            SymEngine::div(
                SymEngine::two,
    ...
    auto val_freq_aab = SymEngine::immutable_dense_matrix(
        2, 2,
        {
            SymEngine::div(SymEngine::one, SymEngine::add({omega_a, omega_a, omega_b})),
            SymEngine::div(
                SymEngine::one,
                SymEngine::sub(SymEngine::add({omega_a, omega_a, omega_b}), E_01)
            ),
