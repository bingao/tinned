...
#include "Tinned/perturbations/PertMultichain.hpp"
...
namespace Tinned
{
    ...
    // Stringify `PertMultichain`
    inline std::string stringify(const PertMultichain& perturbations)
    {
        auto str_perturbations = std::string("[");
        for (const auto& p: perturbations) str_perturbations += stringify(p) + ", ";
        return str_perturbations + std::string("]");
    }
